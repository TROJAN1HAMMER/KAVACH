...
# Single-line comment markers across SCANNABLE_EXTENSIONS' languages. A line
# whose first non-whitespace characters are one of these is never executable
# code, so PATTERN_RULES must not test it — otherwise a comment that merely
# *names* a vulnerable call (e.g. "# INSECURE: ... os.system (CWE-78)") can
# satisfy a `foo\s*\(` pattern on its own and produce a phantom finding
# alongside the real one on the code line below it.
            ...
            try:
                ...
                for rule in PATTERN_RULES:
                    for line_num, line in enumerate(lines, start=1):
                        if line.lstrip().startswith(_LINE_COMMENT_PREFIXES):
                            continue
