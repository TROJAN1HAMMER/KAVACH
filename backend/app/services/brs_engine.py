...
def calculate_brs(findings: list[RawFinding]) -> BRSResult:
    ...
    # Normalize: Cap at reasonable maximum and convert to 0-100 scale
    # Raw BRS can exceed 100 for many findings — we normalize per finding average
    avg_brs = total_brs / len(findings) if findings else 0.0
    normalized_brs = round(min(avg_brs, 100.0), 2)
