"""
KAVACH — Attack Surface Exposure Module
Heuristic-based attack-surface exposure scoring for banking applications.

Formerly called "Zero-Day Prediction" — renamed because that phrasing
claimed a predictive capability (a probability of undiscovered/zero-day
exploitation) this heuristic model was never actually able to support in
front of a technical review panel. What it actually measures is a
composite index of factors *correlated with* exposure to undiscovered
vulnerabilities — attack surface size, dependency hygiene, and
configuration risk — not a forecast of any specific future event. The
score and its explanatory text should always be read that way: a
measured exposure index, not a prediction.
...
Output: AttackSurfaceExposureResult (risk_score, risk_level, factors)
"""
...
@dataclass
class AttackSurfaceExposureResult:
...
def calculate_attack_surface_exposure(
    findings: list[RawFinding],
    sbom: Optional[dict] = None,
) -> AttackSurfaceExposureResult:
    """
    Heuristic attack-surface exposure scorer.
    ...
    Returns:
        AttackSurfaceExposureResult with score, level, and contributing factors.
    """
    ...
    logger.debug("attack_surface_exposure.factor.risky_pkgs", count=risky_pkgs, score=risky_score)

    # ── Factor 5: Configuration Risk ──
    config_score = _config_risk_factor(findings)
    factors["configuration_risk"] = round(config_score, 2)
    logger.debug("attack_surface_exposure.factor.config", score=config_score)

    # ── Factor 6: Code Vulnerability Density ──
    code_findings = [f for f in findings if f.source in ("semgrep", "semgrep-fallback")]
    ...
    logger.info(
        "attack_surface_exposure.complete",
        score=normalized_score,
        risk_level=risk_level,
        factors=factors,
    )

    return AttackSurfaceExposureResult(
...
def _build_recommendation(score: float, factors: dict[str, float]) -> str:
    ...
    if not lines:
        lines.append("Attack surface exposure is currently low. Maintain regular vulnerability scanning and patch cadence.")
